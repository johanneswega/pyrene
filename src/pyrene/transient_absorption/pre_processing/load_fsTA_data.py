...
def load_fsTA_file(file, error=False):
    ...
    # get shots
    nshots = np.loadtxt(file, usecols=[1], skiprows=skiprows)[0]

    # get time axis 
    data = np.loadtxt(file, skiprows=skiprows, usecols=[0])
    ...
    df = np.loadtxt(file, skiprows=skiprows, usecols=indices)
